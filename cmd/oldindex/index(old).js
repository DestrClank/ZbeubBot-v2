...
client.once('ready', async () => {
    ...
    client.ws.on('INTERACTION_CREATE', async (interaction) => {
        ...
        if (command === 'test') {
            ...
            for (const arg in args) {
                const value = args[arg]
                //reply(interaction, value)
                executeslash(value, serverQueue, guild, member, interaction, voiceChannel)
            }   
        }
    ...
    let errorCommand = new DiscordSlash.CommandBuilder();
    let helpCommand = new DiscordSlash.CommandBuilder();
    let embedCommand = new DiscordSlash.CommandBuilder();
    let sendmessageCommand = new DiscordSlash.CommandBuilder();
    let playCommand = new DiscordSlash.CommandBuilder();
    let playOption = new DiscordSlash.CommandBuilder();
    let queueCommand = new DiscordSlash.CommandBuilder();
    let resumeCommand = new DiscordSlash.CommandBuilder();
    let pauseCommand = new DiscordSlash.CommandBuilder();
    let stopCommand = new DiscordSlash.CommandBuilder();

    let embedOption = new DiscordSlash.CommandBuilder();
    let embedTitle = new DiscordSlash.CommandBuilder();
    let sendmessageOption = new DiscordSlash.CommandBuilder();

    testCommand.setName("test");
    testCommand.setDescription("Une fonction simple de test.");

    errorCommand.setName("error");
    errorCommand.setDescription("z!error, envoie un message d'erreur.");

    helpCommand.setName("help");
    helpCommand.setDescription("z!help, envoie la liste des commandes disponibles.");

    playCommand.setName("playmusic");
    playCommand.setDescription("z!play, joue de la musique via un lien YouTube ou effectue une recherche.");

    queueCommand.setName("queue");
    queueCommand.setDescription("z!queue : affiche la liste de lecture.");

    resumeCommand.setName("resume");
    resumeCommand.setDescription("z!resume : reprend la lecture de la musique.");

    pauseCommand.setName("pause");
    pauseCommand.setDescription("z!pause : met en pause la lecture de la musique.");

    stopCommand.setName("stop");
    stopCommand.setDescription("z!stop : arrête la lecture de la musique.");

    embedCommand.setName("embed");
    embedCommand.setDescription("Envoie le message que vous avez saisi sous la forme d'un \"embed\"");
    
    sendmessageCommand.setName("sendmessage");
    sendmessageCommand.setDescription("Envoie le message que vous avez saisi");
    sendmessageCommand.addChoice("message", "");
    sendmessageCommand.setRequired(true);

    embedOption.setName("message");
    embedOption.setDescription("Ecrivez le message ici.");
    embedOption.setRequired(true);
    embedOption.setType(DiscordSlash.CommandType.STRING);

    embedTitle.setName("titre");
    embedTitle.setDescription("Ecrivez le titre ici.");
    embedTitle.setRequired(false);
    embedTitle.setType(DiscordSlash.CommandType.STRING);

    sendmessageOption.setName("message");
    sendmessageOption.setDescription("Ecrivez le message ici.");
    sendmessageOption.setRequired(true);
    sendmessageOption.setType(DiscordSlash.CommandType.STRING);

    playOption.setName("Musique");
    playOption.setDescription("Saisissez le lien de la vidéo ou une recherche YouTube.");
    playOption.setRequired(true);
    playOption.setType(DiscordSlash.CommandType.STRING);

    embedCommand.addOption(embedOption);
    embedCommand.addOption(embedTitle);
    sendmessageCommand.addOption(sendmessageOption);
    playCommand.addOption(playOption);
    */

    /*
    slash.create(testCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /test a été crée ou mis à jour avec succès.");
    })

    slash.create(errorCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /error a été crée ou mis à jour avec succès.");
    })

    slash.create(helpCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /help a été crée ou mis à jour avec succès.");
    })

    slash.create(embedCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /embed a été crée ou mis à jour avec succès.");
    })

    slash.create(sendmessageCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /sendmessage a été crée ou mis à jour avec succès.");
    })
    */
    /*
    slash.create(playCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /playmusic a été crée ou mis à jour avec succès.");
    })

    slash.create(queueCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /queue a été crée ou mis à jour avec succès.");
    })

    slash.create(resumeCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /resume a été crée ou mis à jour avec succès.");
    })

    slash.create(pauseCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /pause a été crée ou mis à jour avec succès.");
    })

    slash.create(stopCommand, guildId).then((res) => {
        console.log("Initialiastion : La commande slash /stop a été crée ou mis à jour avec succès.");
    })
...
slash.on("slashInteraction", async (interaction) => {
    ...
    switch (interaction.command.name) {
        ...
        case "error":
            ...
            break;
        case "queue":
            const guilda = await client.guilds.cache.get(interaction.channel.guild.id);
            const membera = await guilda.members.cache.get(interaction.author.id);
            //const serverQueue = queue.get(interaction.channel.guild.id);
            showQueueSlash(interaction, serverQueue, membera).catch(() => {
                console.log("Fonction slash : La commande /queue a échouée. Cela est dû à une mauvaise optimisation réseau ou du lag sur la connexion.")
                console.log("Il est possible que le bot soit en train de s'installer correctement.")
                console.log("Pour un meilleur résultat, l'utilisateur doit se connecter à un salon vocal avant d'effectuer la commande.")
                return interaction.callback("La commande a échouée... 🥲 Pour que cela fonctionne correctement, connectez-vous dans un salon vocal et cela devrait marcher ! Si cela ne marche pas, utilisez \`z!queue\` à la place.")
            })
            break;
        case "resume":
            const guildb = await client.guilds.cache.get(interaction.channel.guild.id);
            const memberb = await guildb.members.cache.get(interaction.author.id);
            
            resumeMusicSlash(interaction, serverQueue, memberb).catch(() => {
                console.log("Fonction slash : La commande /resume a échouée. Cela est dû à une mauvaise optimisation réseau ou du lag sur la connexion.")
                console.log("Il est possible que le bot soit en train de s'installer correctement.")
                console.log("Pour un meilleur résultat, l'utilisateur doit se connecter à un salon vocal avant d'effectuer la commande.")
                return interaction.callback("La commande a échouée... 🥲 Pour que cela fonctionne correctement, connectez-vous dans un salon vocal et cela devrait marcher ! Si cela ne marche pas, utilisez \`z!resume\` à la place.")
            })
            break;
        case "pause":
            const guildc = await client.guilds.cache.get(interaction.channel.guild.id);
            const memberc = await guildc.members.cache.get(interaction.author.id);
            //const serverQueue = queue.get(interaction.channel.guild.id);
            pauseMusicSlash(interaction, serverQueue, memberc).catch(() => {
                console.log("Fonction slash : La commande /pause a échouée. Cela est dû à une mauvaise optimisation réseau ou du lag sur la connexion.")
                console.log("Il est possible que le bot soit en train de s'installer correctement.")
                console.log("Pour un meilleur résultat, l'utilisateur doit se connecter à un salon vocal avant d'effectuer la commande.")
                return interaction.callback("La commande a échouée... 🥲 Pour que cela fonctionne correctement, connectez-vous dans un salon vocal et cela devrait marcher ! Si cela ne marche pas, utilisez \`z!pause\` à la place.")
            })
            break;
        case "stop":
            const guildd = await client.guilds.cache.get(interaction.channel.guild.id);
            const memberd = await guildd.members.cache.get(interaction.author.id);
            //const serverQueue = queue.get(interaction.channel.guild.id);
            stopSlash(interaction, serverQueue, memberd).catch(() => {
                console.log("Fonction slash : La commande /stop a échouée. Cela est dû à une mauvaise optimisation réseau ou du lag sur la connexion.")
                console.log("Il est possible que le bot soit en train de s'installer correctement.")
                console.log("Pour un meilleur résultat, l'utilisateur doit se connecter à un salon vocal avant d'effectuer la commande.")
                return interaction.callback("La commande a échouée... 🥲 Pour que cela fonctionne correctement, connectez-vous dans un salon vocal et cela devrait marcher ! Si cela ne marche pas, utilisez \`z!stop\` à la place.")
            })
            break;
        default:
    }
...
async function stopSlash(message, serverQueue, member) {
    const voiceChannel = await member.voice.channel;

    if (!voiceChannel) // on vérifie que l'utilisateur est bien dans un salon vocal pour skip
    {
        console.log("Fonction slash stop : L'utilisateur n'était pas dans un salon vocal avant de faire la commande stop.");
    ...
    if (!serverQueue) // On vérifie si une musique est en cours
    {
        console.log("Fonction slash stop  : Aucune musique était en cours de lecture avant de faire la commande stop.");
        return message.callback("Aucune musique est en cours de lecture.");
    }

    if (!serverQueue.playing) {
        //const dispatcher = serverQueue.connection
        console.log("Fonction slash stop  : La musique étant en pause, le processus d'arrêt sera plus long."); //LOGIK
        //serverQueue.playing = true;
        serverQueue.connection.dispatcher.resume();
        //serverQueue.volume = 0;
        //serverQueue.connection.dispatcher.setVolume(0);
    ...
    serverQueue.songs = [];
    client.user.setActivity("z!help", { type: "PLAYING" });
    console.log("Fonction slash stop  : Arrêt de la musique et nettoyage de la liste de lecture.");
    setTimeout(() => {
        serverQueue.connection.dispatcher.end();
    }, 0);
    message.callback("Musique arrêtée ! 😉");
}
...
async function showQueueSlash(interaction, serverQueue, member) {
    const voiceChannel = await member.voice.channel;

    if (!voiceChannel) {
        console.log("Fonction slash queue : L'utilisateur n'était pas dans un salon vocal avant de faire la commande queue.");
        return interaction.callback("Soyez dans un salon vocal pour que cela fonctionne !😉");
    }

    if (!serverQueue) {
        console.log("Fonction slash queue : Aucune musique n'était en cours de lecture avant de faire la commande queue.");

        return interaction.callback("Aucune musique est en cours de lecture.");
    }


    console.log("Fonction slash queue : Affichage de la liste de lecture.");
    let nowPlaying = serverQueue.songs[0];
    let msg = `Musique en cours : **${nowPlaying.title}**\n\n`
...
function pauseMusicSlash(message, serverQueue, member) {
    const PikachuEmote = client.emojis.cache.find(emoji => emoji.name === "Pikachu");
    if (!member.voice.channel) {
        console.log("Fonction slash pause : L'utilisateur n'était pas dans un salon vocal avant de faire la commande pause.");
        return message.callback("Soyez dans un salon vocal pour que cela fonctionne ! 😉");
    }

    if (!serverQueue) {
        console.log("Fonction slash pause : Aucune musique n'était en cours de lecture avant de faire la commande pause.");
        return message.callback("Aucune musique est en cours de lecture.");
    }

    if (!serverQueue.playing) {
        console.log("Fonction slash pause : La musique est déja en pause.");
        return message.callback(`${PikachuEmote} La musique est déja en pause ! 😅`);
    }

    serverQueue.playing = false;
    serverQueue.connection.dispatcher.pause();
    console.log("Fonction slash pause : La musique est en pause.");
    return message.callback(`${PikachuEmote} La musique est en pause ! 😉`);
}
...
async function resumeMusicSlash(message, serverQueue, member) {
    const voiceChannel = await member.voice.channel;
    const PikachuEmote = client.emojis.cache.find(emoji => emoji.name === "Pikachu");
    if (!voiceChannel) {
        console.log("Fonction slash resume : L'utilisateur n'était pas dans un salon vocal avant de faire la commande resume.");
        return message.callback("Soyez dans un salon vocal pour que cela fonctionne ! 😉");
    }
    ...
    if (serverQueue.playing) {
        console.log("Fonction slash resume : La musique est déja en lecture.");
        return message.callback(`${PikachuEmote} La musique est déja en lecture ! 😅`);
    }

    serverQueue.playing = true;
    serverQueue.connection.dispatcher.resume();
    console.log("Fonction slash resume : La musique est en lecture.");
    return message.callback(`${PikachuEmote} La musique est en lecture ! 😉`);
}
...
async function executeslash(value, serverQueue, guild, member, orig, voiceChannel) {
    const args = value
    ...
    if (!voiceChannel) // Si l'utilisateur n'est pas dans un salon vocal
    {
        ...
        return sendInteraction(orig, "Soyez dans un salon vocal pour que cela fonctionne 😉 !");
    }
    
    if (!args[0]) {
        console.log("Fonction slash playmusic : L'utilisateur n'a pas spécifié d'arguments.");
        return sendInteraction(orig, "Pour utiliser la fonction \`z!play\`, vous devez mettre après \`z!play\` des termes de recherche ou un lien vers une vidéo YouTube spécifique.");
    }
    ...
    if (!validate) {
        ...
        return ytsearchslasha(value, serverQueue, member, `z!play ${args}`, orig);
    }
 ...
 if (!permissions.has("CONNECT") || !permissions.has("SPEAK")) { // Si le bot n'a pas les permissions
     console.log("z!play : Le bot ne possède pas les autorisations nécessaires tels que CONNECT et SPEAK. Il ne peut pas continuer.");
     return sendInteraction(orig, "Le bot ne possède pas les autorisations nécessaires au fonctionnement de la lecture de musique. Vérifiez les rôles de @Zbeub Bot et leurs autorisations.");
 }
    /*
    try {
        var video = await youtube.getVideoById(url)
    ...
    */
    ...
    sendInteraction(orig, "Ajout de la musique à la liste de lecture...")
    ...
    if (!serverQueue) {
        ...
        // On ajoute la queue du serveur dans la queue globale:
        queue.set(orig.guild_id, queueConstruct);
        // On y ajoute la musique
        queueConstruct.songs.push(song);
        ...
        try {
            ...
            play(orig.guild_id, queueConstruct.songs[0], queue);
        }
        catch (err) {
            //On affiche les messages d'erreur si le bot ne réussi pas à se connecter, on supprime également la queue de lecture
            console.log("Fonction slash playmusic : Le bot n'a pas réussi à rejoindre le salon vocal.");
            console.log(`Code d'erreur : ${err}`);
            queue.delete(orig.guild_id);
            return sendMessage(orig, err);
        }
    }
    else {
        serverQueue.songs.push(song);
        ...
        //console.log(serverQueue.songs);
        return sendMessage(orig, `**${song.title}** a été ajoutée dans la liste de lecture ! 😉`);
    }
}

function ytsearchslasha(interaction, serverQueue, member, search, orig) {
    var args = search.trim().split(/ +/g);
    ...
    {
        console.log("Fonction ytsearch : L'utilisateur a quitté le salon vocal pendant la sélection de la musique ou n'était pas dans le salon vocal.");
        return sendMessage(orig, "Soyez dans un salon vocal pour que cela fonctionne 😉 !");
    }

    sendMessage(orig, "Recherche sur YouTube...")
    ...
    yts(args.join(' '), function (err, res) {
        ...
        if (err) {
            console.log("Fonction ytsearch : Une erreur inconnue est survenue.");
            return sendMessage(orig, "Pikachu n'est pas content ! Une erreur étrange s'est produite ! **tête choquée**");
        }
