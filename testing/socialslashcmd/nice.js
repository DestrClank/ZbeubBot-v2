...
module.exports = {
	"nicerandom": function(message) {
		...
		const embedMessageRNG = new Discord.MessageEmbed()
			...
			.setDescription(`**${user.username}#${user.discriminator}** te dit **nice** OnO !`)
			.setImage('https://cdn.discordapp.com/attachments/830828771173990420/922831516772478976/yes-nice.gif');
		return message.reply({ embeds: [embedMessageRNG] })
	}, 
	"nicemember": function(message, arg) {
		const embedMessageRNG = new Discord.MessageEmbed()
			.setColor(values.settings.embedColor)
			.setDescription(`**${arg.username}#${arg.discriminator}** te dit **nice** OnO !`)
			.setImage('https://cdn.discordapp.com/attachments/830828771173990420/922831516772478976/yes-nice.gif');
		return message.reply({ embeds: [embedMessageRNG] })
	}
}
