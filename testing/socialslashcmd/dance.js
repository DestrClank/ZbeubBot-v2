...
module.exports = {
	"dancerandom": function(message) {
		...
		const embedMessageRNG = new Discord.MessageEmbed()
			...
			.setDescription(`Pikachu fait une danse à **${user.username}#${user.discriminator}** !`)
	...
	"dancemember": function(message, arg) {
		const embedMessageRNG = new Discord.MessageEmbed()
			.setColor(values.settings.embedColor)
			.setDescription(`Pikachu fait une danse à **${arg.username}#${arg.discriminator}** !`)
			.setImage("https://cdn.discordapp.com/attachments/872866306473472040/872867350658039869/pika_dance.gif");
		return message.reply({ embeds: [embedMessageRNG] })
	}
}
